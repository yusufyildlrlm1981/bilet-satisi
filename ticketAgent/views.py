    ...
    if form.is_valid():
        if loginCheck(form.clean_email(), form.clean_password()):
            ...
            if userEmail != '':
                ...
                context = {
                    "form": form,
                    "title": "LÜTFEN ÖNCE GÝRÝÞ YAPINIZ!"
                }
        else:
            context = {
                "form": form,
                "title": "KULLANICI VEYA PAROLA YANLIÞ!"
            }
...
def signup(request):
    ...
    if form.is_valid():
        try:
            ...
            context = {
                "warning": "Farklý E-posta Kullan, yazdýðýnýz e-posta zaten kayýtlý!",
                "form": form
            }
                ...
                if len(str(ticketReserved)) > 0: 
                    try:
                        if float(ticketCapacities[counter]) >= float(ticketReserved):
                            ...
                            context = {
                                "query_results": query_results,
                                "warning": "Kapasiteden daha fazla bilet satýn alamazsýnýz! Lütfen kalan bilet miktarýnýn altýndaki bir sayý giriniz!"
                            }
                            return render(request, "events/event.html", context)
                    except ValueError:
                        context = {
                            "query_results": query_results,
                            "warning": "Rakamlardan baþka bir karakter yazmayýn!",
                        }
                        return render(request, "events/event.html", context)
        return render(request, "events/event.html", context)
    else:
            ...
            try:
                if float(deletedReservation[0]) > float(request.POST.get("partialInput%s" % resID)):
                    ...
                    context = {
                        "query_results": query_results_main,
                        "email": userEmail,
                        "warning": "Ayrýldýðýndan daha fazla rezervasyon iptal edemezsin!"
                    }
...
def filterEvents(event, begin_date, end_date, showroom, city):
    if event == 'Tümü' or event == '':
        event = '.*'  
    if showroom == 'Tümü' or showroom == '':
        showroom = '.*'  
    if city == 'Tümü' or city == '':
        city = '.*'   
    if begin_date == '':
        begin_date = '01-01'   
    if end_date == '':
    ...
    Where Type REGEXP '%s' and Date between '2019-%s' and '2019-%s') as activity
    join
    (Select *
